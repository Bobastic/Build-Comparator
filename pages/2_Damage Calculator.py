...
with columns[0]:
    ...
    cols=st.columns(4)
    with cols[0]: st.number_input("Magic defense",0,400,st.session_state.defmagic,key="defmagic_",on_change=updateState,args=("defmagic_",))
    with cols[1]: st.number_input("Fire defense",0,400,st.session_state.deffire,key="deffire_",on_change=updateState,args=("deffire_",))
    with cols[2]: st.number_input("Lightning defense",0,400,st.session_state.deflightning,key="deflightning_",on_change=updateState,args=("deflightning_",))
    with cols[3]: st.number_input("Holy defense",0,400,st.session_state.defholy,key="defholy_",on_change=updateState,args=("defholy_",))
    st.divider()
    cols=st.columns(4)
    ...
    cols=st.columns(4)
    with cols[0]: st.number_input("Magic negation",0.,100.,st.session_state.negmagic,format="%.1f",key="negmagic_",on_change=updateState,args=("negmagic_",))
    with cols[1]: st.number_input("Fire negation",0.,100.,st.session_state.negfire,format="%.1f",key="negfire_",on_change=updateState,args=("negfire_",))
    with cols[2]: st.number_input("Lightning negation",0.,100.,st.session_state.neglightning,format="%.1f",key="neglightning_",on_change=updateState,args=("neglightning_",))
    with cols[3]: st.number_input("Holy negation",0.,100.,st.session_state.negholy,format="%.1f",key="negholy_",on_change=updateState,args=("negholy_",))
