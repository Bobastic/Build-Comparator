...
st.sidebar.info("Default enemy defenses are the average of classic STR, DEX, INT, FTH and ARC builds. Default negations are Imp Head/Beast Champion Armor (Altered)/Lionel's Gauntlets/Lionel's Greaves.")
...
cols=st.columns(8)
with cols[0]: st.number_input("Standard negation",0.,100.,st.session_state.negstandard,key="NEGSTANDARD",on_change=updateState,args=("NEGSTANDARD",))
with cols[1]: st.number_input("Strike negation",0.,100.,st.session_state.negstrike,key="NEGSTRIKE",on_change=updateState,args=("NEGSTRIKE",))
with cols[2]: st.number_input("Slash negation",0.,100.,st.session_state.negslash,key="NEGSLASH",on_change=updateState,args=("NEGSLASH",))
with cols[3]: st.number_input("Pierce negation",0.,100.,st.session_state.negpierce,key="NEGPIERCE",on_change=updateState,args=("NEGPIERCE",))
with cols[4]: st.number_input("Magic negation",0.,100.,st.session_state.negmagic,key="NEGMAGIC",on_change=updateState,args=("NEGMAGIC",))
with cols[5]: st.number_input("Fire negation",0.,100.,st.session_state.negfire,key="NEGFIRE",on_change=updateState,args=("NEGFIRE",))
with cols[6]: st.number_input("Lightning negation",0.,100.,st.session_state.neglightning,key="NEGLIGHTNING",on_change=updateState,args=("NEGLIGHTNING",))
with cols[7]: st.number_input("Holy negation",0.,100.,st.session_state.negholy,key="NEGHOLY",on_change=updateState,args=("NEGHOLY",))
