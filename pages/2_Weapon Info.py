    ...
    with cols[0]:
        ...
        for i in range(8):
            fig.add_trace(go.Bar(x=data[:,i],y=labels,name=dmgTypes[i],orientation="h",
                                 text=[round(d) for d in data[:,i]],marker={"color":colors[i]},showlegend=bool(data[:,i].sum()!=0),insidetextfont={"size":13},textangle=0))
        ...
        fig.update_layout(barmode='stack',legend_traceorder="normal",legend={"orientation":"h","yanchor":"bottom","y":1.02}) #autosize=False,height=350
